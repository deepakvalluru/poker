...
public class TwoPair
{
   public static Result isValid( SetOfCards setOfCards )
   {
      ...
      if( list != null && list.size() >= 2 )
      {
         ...
         List<Integer> topTwo =
                  list.stream().sorted( Collections.reverseOrder() ).limit( 2 ).collect( Collectors.toList() );
         topTwo.stream().forEachOrdered( x -> cards.addCards( cardMap.get( x ) ) );

         Optional<Integer> fifthCardNumber =
                  cardMap.keySet().stream().filter( x -> !topTwo.contains( x ) ).max( Collections.reverseOrder() );
